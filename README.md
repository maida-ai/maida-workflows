...
execute_maida = celery_app.task(name="acme.execute_maida")(CeleryBackend.task_handler(worker_for))
...
`worker_for` is trusted application code that resolves the exact workflow,
definition digest, and module from local registries. Generated bytes cannot
...
its trusted handler loads the authoritative envelope. Downstream tasks are
...
external runtime while retaining the same compare-and-swap accepted boundary.
