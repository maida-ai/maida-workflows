ALTER TABLE workflow_tasks
    ...
    ADD COLUMN plan_revision INTEGER CHECK (plan_revision IS NULL OR plan_revision > 0),
    ADD CONSTRAINT workflow_tasks_plan_provenance_complete CHECK (
        (parent_task_id IS NULL AND plan_region_id IS NULL
         ...
         AND plan_digest IS NULL AND plan_revision IS NULL)
        OR
        (parent_task_id IS NOT NULL AND plan_region_id IS NOT NULL
         ...
         AND plan_digest IS NOT NULL AND plan_revision IS NOT NULL)
    );

CREATE UNIQUE INDEX workflow_tasks_dynamic_node_idx
    ON workflow_tasks (run_id, plan_region_instance_id, plan_revision, plan_node_key)
    WHERE plan_region_instance_id IS NOT NULL;

CREATE INDEX workflow_tasks_dynamic_region_idx
    ON workflow_tasks (run_id, plan_region_instance_id, plan_revision, status)
    WHERE plan_region_instance_id IS NOT NULL;
