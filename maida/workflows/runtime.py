...
from .model import ModelAdapterRegistry, ModelBroker, ModelSpec, _model_contract
from .models import (
...
class DurableRuntimeStore(Protocol):
    ...
    def adopt_run_definition(
        ...
        *,
        ...
        """Replace a bootstrap definition with an accepted generated root plan."""
        ...
...
def _bootstrap_plan(planner: Module[Any, Any]) -> PlanIR:
    ...
    step = StepIR(
        ...
        kind="module",
        dependencies=("input",),
        output_schema_digest=schema_digest(planner.output_type),
        module_id=module_id,
        ...
        input_binding=BindingIR(
            schema_digest=schema_digest(planner.input_type),
            kind="source",
            source="input",
        ),
        execution=planner.execution.to_data(),
        capabilities=access["capabilities"],
        effects=access["effects"],
        models=_model_contract(planner),
        budget=planner.budget.to_data(),
    )
    return PlanIR(
        version=IR_VERSION,
        workflow_id=f"bootstrap:{module_id}",
        input_schema=type_schema(planner.input_type),
        output_schema=type_schema(planner.output_type),
        steps=(step,),
...
class WorkflowRunner:
    ...
    async def run_generated[InputT, OutputT](
        self,
        planner: Module[InputT, dict[str, Any]],
        value: InputT,
        *,
        tenant_id: str = "local",
        execution_mode: ExecutionMode = ExecutionMode.LIVE,
        policy: AssertionPolicy | None = None,
        plan_baseline: Mapping[str, Any] | None = None,
    ) -> RunResult:
        """Generate, validate, execute, and complete one root plan.

        ``planner`` must carry an application-owned :class:`PlanBoundary`
        declaration. The runtime executes that module as a durable bootstrap,
        adopts its validated result as the run definition, materializes every
        child transactionally, and derives the run result only from generated
        child outputs.

        Generated bytes remain limited to node keys, allowlisted aliases,
        dependencies, and outputs. The marker supplies all identities, schemas,
        grants, budgets, and execution requirements from trusted code.

        When a core Maida policy is supplied, its plan rules and optional core
        baseline are evaluated after trusted resolution and before definition
        adoption or child insertion. Approved evidence and the post-execution
        conformance proof are retained in the durable event history.
        """
        marker = getattr(planner, "plan_boundary", None)
        if not isinstance(marker, PlanBoundary):
            raise RuntimeContractError("planner module must declare a trusted PlanBoundary")
        if not value_matches_type(value, planner.input_type):
            raise RuntimeContractError("root input violates the planner input contract")

        bootstrap = _bootstrap_plan(planner)
        root_input = self.store.values.encode(
            value, schema_digest=schema_digest(planner.input_type)
        )
        ...
        planner_step = bootstrap.executable_steps[0]
        source = self.store.enqueue_task(
            run.run_id,
            planner_step,
            step_instance_id=digest_data(
                {"definition_digest": bootstrap.digest, "node_id": planner_step.node_id}
            )[:24],
        ...
        try:
            ...
            validator = PlanValidator(
                ...
                region_grant=marker.region_grant,
            )
            expected_outputs = (schema_digest(marker.output_type),)
            signature = validator.validate(
                fragment,
                region_input_schema_digest=source.accepted_boundary.input_schema_digest,
                expected_output_schema_digests=expected_outputs,
            )
            from .guardrail import PlanGuardrail, PlanGuardrailError

            guardrail = PlanGuardrail(policy=policy, baseline=plan_baseline)
            ...
            if not evidence.valid:
                ...
                raise PlanGuardrailError(evidence, guardrail.format(evidence))
            self.store.append_event(
                run.run_id,
                "PLAN_APPROVED",
                evidence.to_dict(),
            ...
            from .materialization import DynamicPlanScheduler, PlanMaterializer

            region_instance_id = f"{marker.region_id}-root"
            PlanMaterializer(cast(Any, self.store), marker.registry).materialize(
                run_id=run.run_id,
                tenant_id=tenant_id,
                source_task_id=source.task_id,
            ...
            scheduler = DynamicPlanScheduler(
                cast(Any, self.store),
                marker.registry,
                run_id=run.run_id,
                region_instance_id=region_instance_id,
            ...
            while True:
                progress = scheduler.advance()
                failed = tuple(task for task in progress.tasks if task.status is TaskStatus.FAILED)
                if failed:
                    if last_error is not None:
                        raise last_error
                    raise RuntimeExecutionError(
                        f"generated task {failed[0].task_id} failed permanently"
                    )
                if progress.complete:
                    if len(progress.outputs) != 1:
                        raise RuntimeContractError(
                            "generated root plan must produce exactly one output"
                        )
                    output = progress.outputs[0]
                    if not value_matches_type(output, marker.output_type):
                        raise RuntimeContractError(
                            "generated root output violates its trusted contract"
                        )
                    root_output = self.store.values.encode(
                        output,
                        schema_digest=schema_digest(marker.output_type),
                    )
                    history = self.store.load_run_history(run.run_id, tenant_id=tenant_id)
                    ...
                    if issues:
                        ...
                        raise guardrail.execution_error(issues)
                    self.store.append_event(
                        run.run_id,
            ...
            self.store.fail_run(run.run_id, diagnostic)
            raise
